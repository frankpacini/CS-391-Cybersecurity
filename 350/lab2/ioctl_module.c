...
static int __init initialization_routine(void) {
  ...
  pseudo_dev_prov_operations.read = pseudo_device_read;
...
static ssize_t pseudo_device_read(struct file *file, char __user *__user, size_t count, loff_t *offset) 

{
  char *message;
  
  copy_from_user(message, __user, count);
  printk("<1> read: recieved \"%s\"!\n", message);
